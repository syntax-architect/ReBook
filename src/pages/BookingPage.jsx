...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      ...
      try {
        ...
        if (!error && data) {
          setExistingBookings(data.filter((b) => b.status !== 'cancelled'));
        }
      } catch (err) {
    ...
    fetchBookings();
  }, [shop, selectedDate, dates]);

  const availableTimes = useMemo(() => {
    if (!shop || !selectedService) return [];

    let workingHours = shop.working_hours || { open: '09:00', close: '18:00' };

    const dayOfWeek = dates[selectedDate].fullDate
    ...
    if (workingHours.closed || workingHours.is_closed) return [];

    const [openHour, openMin] = (workingHours.open || '09:00').split(':').map(Number);
    const [closeHour, closeMin] = (workingHours.close || '18:00').split(':').map(Number);
    ...
    const bookingsTimeRanges = existingBookings.map((b) => {
      const start = new Date(b.scheduled_at);
      const bService = services.find((s) => s.id === b.service_id);
      const duration = bService ? bService.duration_minutes : 30;
    ...
    while (currentSlot.getTime() + serviceDuration * 60000 <= endOfDay.getTime()) {
      ...
      if (slotStart > now) {
        const hasOverlap = bookingsTimeRanges.some((range) => {
    ...
    return slots;
  }, [shop, selectedService, existingBookings, dates, selectedDate, services]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center font-label-md text-on-surface">
    ...
    return (
      <div className="min-h-screen bg-surface flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="font-body-md text-body-md text-on-surface-variant max-w-md">
          The booking link you followed doesn't seem to exist. Please verify the link with the
          business owner.
        </p>
      </div>
    );
  ...
  if (isSubscriptionInactive) {
    return (
      <div className="min-h-screen bg-surface flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="font-body-md text-body-md text-on-surface-variant max-w-md">
          Online booking is currently paused for {shop.name}. Please contact them directly to book
          an appointment.
        </p>
      </div>
    );
  ...
  return (
    <>
      <div className="">
        <main className="w-full bg-surface min-h-screen px-margin">
          <div className="min-h-screen py-8 sm:py-12 px-4 sm:px-6 flex flex-col items-center justify-between">
            ...
            <header className="w-full max-w-2xl mb-6 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <span className="hidden sm:inline-flex items-center gap-1 text-label-sm font-label-sm text-secondary bg-secondary-container/40 px-3 py-1 rounded-full">
                  <span className="w-1.5 h-1.5 rounded-full bg-secondary"></span> Instant WhatsApp
                  Booking
                </span>
                <button
                  type="button"
            ...
            <main className="w-full max-w-2xl bg-surface-container-lowest rounded-2xl shadow-xl border border-outline-variant/60 overflow-hidden relative">
              ...
              <div className="p-6 bg-surface-container-low border-b border-outline-variant/60">
                  ...
                  <div className="flex items-start gap-3.5">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-body-sm text-body-sm text-on-surface-variant capitalize">
                        {shop.industry} • Verified Partner
                      </span>
                ...
                <div className="mt-4 bg-secondary-container/30 px-3 py-2 rounded-xl flex items-center gap-2">
                  ...
                  <span className="font-label-sm text-label-sm text-on-secondary-container font-medium">
                    Instant WhatsApp confirmation • Pay zero advance at booking
                  </span>
                </div>
              </div>
              ...
              <div className="p-6 sm:p-8 space-y-8">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <span className="w-6 h-6 rounded-full bg-primary-container text-on-primary font-label-sm text-label-sm flex items-center justify-center font-bold">
                        1
                      </span>
                    ...
                    <span className="font-label-sm text-label-sm text-on-surface-variant font-medium">
                      {selectedService ? '1 service selected' : '0 selected'}
                    </span>
                  </div>
                    ...
                    {services.map((service) => {
                      ...
                      return (
                        <div
                          ...
                          className={`relative p-4 rounded-2xl transition-all cursor-pointer ${isSelected ? 'bg-surface-container-low border-2 border-primary shadow-sm' : 'bg-surface-container-lowest hover:bg-surface-container-low/60 border border-outline-variant/60 shadow-sm'}`}
                        >
                          <div className="flex items-start justify-between gap-4">
                            ...
                            <div
                              className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${isSelected ? 'bg-primary text-on-primary shadow-sm' : 'border-2 border-outline-variant'}`}
                            >
                              {isSelected && (
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-label-sm text-label-sm text-primary font-semibold flex items-center gap-1">
                      ...
                      {dates[selectedDate].fullDate.toLocaleString('default', {
                        month: 'long',
                        year: 'numeric',
                      })}
                    </span>
                    ...
                    {dates.map((dateObj) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex flex-col items-center py-3 px-2 rounded-xl transition-transform active:scale-95 border ${isSelected ? 'bg-primary-container text-on-primary shadow-sm border-transparent' : 'bg-surface-container-low hover:bg-surface-container text-on-surface border-transparent hover:border-outline-variant/40'}`}
                          type="button"
                        >
                          {dateObj.offset === 0 ? (
                            <span className="px-2 py-0.5 rounded-full bg-white/20 font-label-sm text-[10px] mb-1 font-medium">
                              Today
                            </span>
                          ) : (
                          ...
                          <span
                            className={`font-label-sm text-label-sm font-medium ${isSelected ? 'text-primary-fixed' : 'text-on-surface-variant'}`}
                          >
                            {dateObj.offset === 0 && !isSelected ? 'Today' : dateObj.dayName}
                          </span>
                          <span className="font-headline-md text-headline-md leading-none mt-1 font-bold">
                            {dateObj.dayNumber}
                          </span>
                        </button>
                      );
                      ...
                      {availableTimes.map((time) => {
                        ...
                        return (
                          <button
                            ...
                            className={`py-2.5 px-2 rounded-xl font-label-sm text-label-sm font-semibold flex items-center justify-center gap-1.5 ${isSelected ? 'bg-primary text-on-primary shadow-sm' : 'bg-surface-container-low hover:bg-surface-container text-on-surface border border-transparent hover:border-outline-variant/40'}`}
                            type="button"
                          >
                            {isSelected && (
                  ...
                  <div className="p-3.5 rounded-xl bg-[#DCF8C6]/60 border border-[#DCF8C6] flex items-start gap-2.5 shadow-xs">
                    ...
                    <p className="font-body-sm text-body-sm text-[#0F172A]">
                      <strong className="font-label-sm font-semibold">
                        Automated WhatsApp Reminder:
                      </strong>{' '}
                      A dispatch alert will arrive 2 hours prior to your slot with 1-click
                      reschedule.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative flex items-center">
                        ...
                        <input
                          ...
                          className="w-full h-11 pl-10 pr-3 rounded-xl bg-surface-container-low border border-outline-variant/50 text-on-surface font-body-md text-body-md focus:bg-surface-container-lowest focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                    ...
                    <div>
                      ...
                      <div className="flex items-center rounded-xl bg-surface-container-low border border-outline-variant/50 overflow-hidden focus-within:bg-surface-container-lowest focus-within:ring-2 focus-within:ring-primary/20">
                        ...
                        <input
                          ...
                          className="w-full h-11 px-3 bg-transparent text-on-surface font-body-md text-body-md focus:outline-none"
                  ...
                  <p className="font-body-sm text-[12px] text-on-surface-variant flex items-center gap-1">
                    ...
                    Zero promotional spam. Solely for calendar pass and appointment arrival updates.
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center justify-between p-3.5 rounded-xl bg-surface-container-lowest border border-outline-variant/60">
                  <div className="flex flex-col">
                    ...
                    <span className="font-body-sm text-body-sm text-on-surface-variant">
                      ...
                      {dates[selectedDate].fullDate.toLocaleString('default', { month: 'short' })} •{' '}
                      {selectedTime || 'Select time'} ({selectedService?.duration_minutes || 0}{' '}
                      mins)
                    </span>
                  </div>
              ...
              {showSuccess && (
                <div
                  className="absolute inset-0 bg-surface-container-lowest z-30 p-6 sm:p-8 flex-col justify-between transition-opacity duration-300 flex"
                  id="booking-success-tray"
                >
                  <div className="space-y-6 pt-4 text-center max-w-md mx-auto">
                    ...
                    <div>
                      ...
                      <p className="font-body-sm text-body-sm text-on-surface-variant mt-1">
                        We dispatched your confirmation ticket to WhatsApp
                      </p>
                    </div>
                    <div className="p-4 rounded-2xl bg-surface-container-low text-left space-y-2.5 border border-outline-variant/60">
                      ...
                      <div className="flex justify-between text-body-sm font-body-sm">
                        ...
                        <span className="font-medium text-secondary font-semibold">
                          {dates[selectedDate].offset === 0 ? 'Today' : dates[selectedDate].dayName}{' '}
                          • {selectedTime}
                        </span>
                      </div>
                      <div className="flex justify-between text-body-sm font-body-sm">
                    ...
                    <div className="p-3.5 rounded-xl bg-[#DCF8C6] text-left border border-[#DCF8C6]">
                      ...
                      <p className="font-body-sm text-body-sm text-on-surface">
                        "Hi {customerName.split(' ')[0]}! Your slot is locked for {selectedTime}{' '}
                        {dates[selectedDate].offset === 0
                          ? 'today'
                          : 'on ' + dates[selectedDate].dayName}{' '}
                        at {shop.name}. Reply CANCEL if plans change."
                      </p>
                    </div>
                  </div>
                  <button
                    ...
                    className="w-full max-w-md mx-auto py-3 rounded-xl bg-surface-container text-on-surface font-label-md text-label-md hover:bg-surface-container-high transition-colors"
                    type="button"
                  >
                    Book Another Slot
                  </button>
            ...
            <footer className="w-full max-w-2xl mt-8 pt-4 border-t border-outline-variant/30 flex flex-col sm:flex-row items-center justify-between text-on-surface-variant font-body-sm text-body-sm gap-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="">
                  Powered by <strong className="font-semibold text-primary">Rebook</strong>
                </span>
              </div>
