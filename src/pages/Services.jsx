...
export default function Services() {
  ...
  const handleToggleStatus = async (id) => {
    ...
    // Optimistic update
    setServices((prev) =>
      prev.map((s) => (s.id === id ? { ...s, status: newIsActive ? 'active' : 'paused' } : s))
    );
  ...
  return (
    <>
      <div className="flex flex-col w-full pb-margin relative">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-space-md mb-space-lg pt-space-md">
          <div className="flex flex-col">
            ...
            <p className="font-body-md text-body-md text-on-surface-variant max-w-2xl">
              Configure treatments, durations, and pricing synced directly to your WhatsApp instant
              booking catalog.
            </p>
          </div>
          <div className="flex items-center gap-space-sm">
            <button
              onClick={() => window.open('https://wa.me/c/your_business_id', '_blank')}
              className="inline-flex items-center gap-1.5 px-space-md py-2 rounded-lg bg-surface-container-lowest text-on-surface-variant hover:text-primary font-label-lg text-label-lg shadow-sm transition-colors"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-space-lg py-2.5 rounded-lg bg-secondary text-on-secondary font-label-lg text-label-lg shadow-sm hover:opacity-95 active:scale-[0.99] transition-all"
            >
              ...
              <AnimatePresence initial={false}>
                {filteredServices.map((service) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`absolute left-0 top-0 bottom-0 w-1 ${isActive ? 'bg-secondary' : 'bg-outline-variant'}`}
                      ></div>

                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-space-md pl-space-xs">
                        <div className="flex items-start gap-space-md">
                          <div
                            className={`w-12 h-12 rounded-xl flex-shrink-0 flex items-center justify-center ${isActive ? service.bgClass : 'bg-surface-container text-outline'}`}
                          >
                          ...
                          <div className="flex flex-col">
                            <div className="flex items-center gap-space-xs flex-wrap">
                              ...
                              {service.tag && (
                                <span
                                  className={`px-2 py-0.5 rounded-full font-label-sm text-label-sm ${isActive ? 'bg-primary-fixed text-on-primary-fixed' : 'bg-surface-container-highest text-on-surface-variant'}`}
                                >
                                  {service.tag}
                                </span>
                              )}
                            ...
                            <div className="flex items-center gap-3 mt-1 font-body-sm text-body-sm text-on-surface-variant">
                              ...
                              {service.bookingsThisWeek > 0 ? (
                                <span className="text-secondary font-label-sm text-label-sm">
                                  {service.bookingsThisWeek} bookings this week
                                </span>
                              ) : (
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-space-lg pt-2 sm:pt-0">
                          ...
                          <div className="flex items-center gap-space-sm">
                            <label className="relative inline-flex items-center cursor-pointer">
                              ...
                              <span
                                className={`ml-2 font-label-sm text-label-sm ${isActive ? 'text-secondary' : 'text-outline'}`}
                              >
                            ...
                            <div className="flex items-center gap-1 text-on-surface-variant ml-1">
                              ...
                              <button
                                ...
                                className="p-1.5 hover:bg-error-container/50 rounded-lg transition-colors text-on-surface-variant hover:text-error"
                                title="Delete"
                              >
              ...
              <div className="flex items-center gap-space-sm">
                ...
                <div className="flex flex-col">
                  <span className="font-label-md text-label-md text-on-surface">
                    Auto-sync with WhatsApp Business Catalog
                  </span>
                  <span className="font-body-sm text-body-sm text-on-surface-variant">
                    Last synchronized automatically 4 minutes ago.
                  </span>
                </div>
              </div>
              <button
                onClick={() => showToast('Forced sync initiated...', 'sync')}
                className="px-3 py-1.5 rounded-lg bg-surface-container-lowest text-primary font-label-md text-label-md shadow-sm hover:bg-white transition-all whitespace-nowrap"
              >
                Force Resync
              </button>
            </div>
            ...
            <div className="bg-surface-container-lowest rounded-xl p-space-lg shadow-sm border border-outline-variant/30">
                ...
                <div className="flex items-center gap-space-sm">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-on-surface tracking-tight">
                      {formData.id ? 'Edit' : 'Add New'} {terms.service}
                    </h2>
                    <p className="text-[11px] text-on-surface-variant/80 mt-0.5">
                      Define pricing, duration, and metadata.
                    </p>
                  </div>
                </div>
                <span
                  className={`w-8 h-8 rounded-full flex items-center justify-center ${formData.id ? 'bg-secondary-container text-secondary' : 'bg-surface-container text-primary'}`}
                >
              ...
              <form className="flex flex-col gap-space-md" onSubmit={handleFormSubmit}>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 font-headline-sm text-headline-sm text-outline">
                      ₹
                    </span>
                    <input
                      required
                ...
                <div className="flex flex-col gap-1.5 pt-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-label-sm text-label-sm text-secondary font-semibold animate-pulse">
                      Live Preview
                    </span>
                  </div>
                  <div className="p-3.5 rounded-xl bg-[#DCF8C6]/50 shadow-inner flex flex-col gap-1">
                    <div className="flex items-start justify-between">
                      ...
                      <span className="font-label-sm text-label-sm text-on-surface-variant tabular-nums ml-2 shrink-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 font-label-sm text-label-sm text-primary-container font-semibold">
                      ...
                      <span className="material-symbols-outlined text-sm text-secondary ml-auto">
                        done_all
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-2 pt-2">
                  <button
                    type="submit"
                    className="w-full h-10 rounded-lg bg-primary-container text-on-primary font-label-lg text-label-lg shadow-sm hover:opacity-95 active:scale-[0.99] transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setFormData(emptyForm)}
                    className="w-full h-9 rounded-lg hover:bg-surface-container text-on-surface-variant font-label-md text-label-md transition-colors text-center"
                  >
                    Clear / Cancel
                  </button>
                </div>
        ...
        <div
          className={`fixed bottom-6 right-6 transform transition-all duration-300 z-50 flex items-center gap-2 bg-on-surface text-surface px-4 py-3 rounded-xl shadow-xl font-label-md text-label-md ${toast.show ? 'translate-y-0 opacity-100' : 'translate-y-24 opacity-0 pointer-events-none'}`}
        >
